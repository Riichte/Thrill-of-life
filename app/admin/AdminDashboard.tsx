...
export default function AdminDashboard({ parks, categories, items }: { parks: Park[]; categories: Category[]; items: Item[] }) {
  ...
  const [itemImages, setItemImages] = useState<{ id: string; url: string; sort_order: number }[]>([])
  ...
  const handleAddImage = async () => {
    ...
    setLoading(true)
    const { error } = await supabase.from('item_images').insert({ item_id: imageItemId, url: imageUrl.trim(), sort_order: imageOrder })
    if (error) notify(error.message, true)
    else { notify('Image added'); setImageUrl(''); loadImages(imageItemId) }
    setLoading(false)
  }
